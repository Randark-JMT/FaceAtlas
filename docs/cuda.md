...
若使用 CUDA 时 GPU 利用率较低（例如只有约 25%），可尝试以下方式提高吞吐与利用率：

1. **并发数**  
   程序在使用 CUDA 时会自动提高检测线程数（多线程向 GPU 提交任务），减少 GPU 等待。若仍觉得偏慢，可在代码中为 `DetectWorker` 显式传入更大的 `num_workers`（例如 8）做测试。

2. **限制检测输入尺寸**  
   大图（如 4K）会拉高单张推理耗时并占用显存。可在配置中限制检测时的输入最长边，使大图先缩小再检测，从而提高吞吐并更易吃满 GPU：
   - 在 AppData 下的 `config.json` 中增加（若文件不存在可先运行一次程序再编辑）：
     ```json
     "detection_input_max_side": 640
     ```
   - 常用取值：`640`（更快、略损大图小脸检出）、`1280`（折中）、`0`（不限制，默认）。
   - 限制后 bbox/关键点会自动换算回原图坐标，不影响后续特征与聚类。

3. **观察 GPU 占用**  
   使用 `nvidia-smi` 或任务管理器查看 GPU 利用率和显存。若利用率明显提升且识别变快，说明上述设置生效。